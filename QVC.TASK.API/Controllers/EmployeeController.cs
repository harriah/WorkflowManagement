            ...
            try
            {
                ...
                if (resdomain?.Count > 0 || resinffo?.Count > 0)
                {
                    if (type == TypeGetEmp.Assign)
                    {
                        ...
                        // result chứa thông tin của 2 nhóm
                        results = employees.GroupBy(e => e.EmployeeID).Select(g => g.First()).ToList();
                    }
                    else if (type == TypeGetEmp.Add)
                    {
                        // sử dụng để lấy danh sách các Employee
                        // muốn "thêm" (Add) vào danh sách từ resinffo,
                        // bằng cách loại bỏ các Employee mà đã tồn tại trong resdomain. 
